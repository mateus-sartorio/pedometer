 ...
 *          The alternative would be the inclusion of the processor specific
 * file directly, e.g. #include "efm32gg995f1024.h"
 *
 *
 * @version 1.0
 * @author  Hans
 ...
 *
 * @note            From Silicon Labs system_efm32gg.c. It is a static
 * (internal) function there.
 */
...
uint32_t ClockGetConfiguration(ClockConfiguration_t *p) {
    ...
    status = CMU->STATUS;
    hclkdivcode =
        (CMU->CTRL & ~(_CMU_CTRL_HFCLKDIV_MASK)) >> _CMU_CTRL_HFCLKDIV_SHIFT;
